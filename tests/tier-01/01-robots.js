...
import AllRobots, { AllRobots as UnconnectedAllRobots } from '../../app/components/AllRobots';
...
describe.only('Tier One: Robots', () => {
	...
	describe('<AllRobots /> component', () => {
		...
		xit('renders the robots passed in as props', () => {
			const wrapper = mount(<UnconnectedAllRobots robots={robots} getRobots={getRobotsSpy} />);
			...
			expect(images).to.include.members([ '/images/r2d2.png', '/images/walle.jpeg' ]);
		});
		...
		xit('renders DIFFERENT robots passed in as props', () => {
			...
			const wrapper = mount(<UnconnectedAllRobots robots={differentRobots} getRobots={getRobotsSpy} />);
			...
			const images = wrapper.find('img').map((node) => node.get(0).props.src);
			expect(images).to.include.members([ '/images/HAL-9000.jpeg', '/images/bender.png' ]);
		});
